...
function key() {
  if (!process.env.TAVILY_API_KEY) throw new Error("未配置 TAVILY_API_KEY，真实搜索没有执行")
  return process.env.TAVILY_API_KEY
}

async function tavilyRequest<T>(path: string, body: object): Promise<T> {
...
export function searchTavily(query: string, maxResults = 6) {
  return tavilyRequest<{ query: string; results: TavilyResult[]; response_time: string; request_id: string }>("/search", {
    query, search_depth: "basic", max_results: Math.min(Math.max(maxResults, 1), 10), include_answer: false, include_raw_content: false,
  })
}

export function extractTavily(urls: string[]) {
